...
class Camera():
    ...
    def __init__(self):
        ...
        self.plan_publisher = rospy.Publisher('/plan', String, queue_size=10)
        hand_grasps_path = rospkg.RosPack().get_path('robot_pnp') + f"/config/numbers.yaml"
        self.config={}
                ...
                for key, value in self.config.items():
                    if value == fingers1:
                        cv2.putText(img, key, (bbox1[0] + bbox1[2] -40, bbox1[1] - 30), cv2.FONT_HERSHEY_SIMPLEX,
                            1, (0, 255, 0), 2)
                        self.plan_publisher.publish(key)
                        break
